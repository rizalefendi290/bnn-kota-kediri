        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white dark:bg-gray-800 mx-auto">
                ...
                <tbody>
                            ...
                            <td class="py-2 px-4">
                                <form action="{{ route('pengaduan.updateStatus', $pengaduan->id) }}" method="POST">
                                    ...
                                    <select name="status" onchange="this.form.submit()"
                                        class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-700 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md dark:bg-gray-900 dark:text-white">
                                        <option value="belum_diproses" {{ $pengaduan->status == 'belum_diproses' ? 'selected' : '' }}>Belum Diproses</option>
                                        <option value="proses" {{ $pengaduan->status == 'proses' ? 'selected' : '' }}>Proses</option>
                                        <option value="sudah_diproses" {{ $pengaduan->status == 'sudah_diproses' ? 'selected' : '' }}>Sudah Diproses</option>
                                    </select>
                                </form>
                            </td>
                            <td class="py-2 px-4">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="py-4 px-6 text-center">No complaints found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
